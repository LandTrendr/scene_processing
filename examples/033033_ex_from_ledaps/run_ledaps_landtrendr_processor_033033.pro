...
useareafile = "/vol/v1/scenes/gnn_snapped_cmon_usearea_files/033033_usearea.bsq" ; full path to a 0-1 raster that defines where Landtrendr will be run :: zeros = don't run, ones = run 
segparamstxt = "/vol/v1/general_files/helperfiles/cmonster_nbr_segmentation_parameters.txt" ;give full path to the segmentation parameter .txt file
template_hdr = "/vol/v1/general_files/helperfiles/mrlc_template_headerfile.hdr"  ;give the full path to a template projection header (.hdr) file
label_parameters_txt = ""
class_code_txt = ""
...
segmentation         = 1  ;creates segmentation outputs
fit_to_vertices      = 5  ;use 1 to run bgw, 2 to run b5,b4,b3, 3 to run both, 4 to run nbr, 5 to run nbr + bgw, etc.
dist_rec_snapshots   = 0  ;create disturbance and recovery slice outputs
dark_seg_outputs     = 1  ;creates an output used to make a forest-nonforest mask
