...
ppprrr = '033033' ; ex. '046026' -MUST BE PPP (path) and RRR (row)
;three digits for both path and row, use zero if needed ex: '0PP0RR'
path = '/projectnb/trenders/scenes/033033/' ; ex. 'F:\046026\' -MAKE SURE THERE IS A "\" AT THE END, must point to a drive path, even if on a server  
useareafile = "/projectnb/trenders/scenes/gnn_snapped_cmon_usearea_files/033033_usearea.bsq" ; full path to a 0-1 raster that defines where Landtrendr will be run :: zeros = don't run, ones = run 
segparamstxt = "/projectnb/trenders/general_files/helperfiles/nbr_segmentation_parameters.txt" ;give full path to the segmentation parameter .txt file
template_hdr = "/projectnb/trenders/general_files/helperfiles/mrlc_template_headerfile.hdr"  ;give the full path to a template projection header (.hdr) file
label_parameters_txt = "/projectnb/trenders/general_files/helperfiles/eval_label_params.txt"
class_code_txt = "/projectnb/trenders/general_files/helperfiles/eval_class_codes.txt"
...
fit_to_vertices      = 0  ;use 1 to run bgw, 2 to run b5,b4,b3, 3 to run both, 4 to run nbr, 5 to run nbr + bgw, etc.
